...
sub SetOrder
{
	...
	$average_per_position /= ($cfg->{$section}->{'width'} - $site_size);
...
sub CountFromFile
{
	...
	while( my $line = <IN>)
	{
		...
		if ( $site_type )
		{
			if ( $cfg->{$section}->{'force'} )
			{
				substr( $seq, $site_margin, $site_size ) = $site_type;
			}

			if ( $site_type ne substr( $seq, $site_margin, $site_size) )
			{
				print "warning, unexpected site was found and sequence ignored: $lines_read_in, $line" if !$quiet;
				next;
			}
		}
...
optional:
  ...
  --format_out     [label]
  --quiet          ignore non-canonical sites without warnings
  --force          force site type on all input sequence
